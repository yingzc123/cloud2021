...
import com.yzc.springcloud.entity.dto.enumAll.MsgEnum;
...
public class LoginLogServiceImpl extends ServiceImpl<LoginLogMapper, LoginLog> implements LoginLogService {
    ...
    @Override
    public User login(LoginDto.LoginPwd loginLoginPwd) {
        userService.existNumber(loginLoginPwd.getUserNumber());
        User user = userService.getOne(new QueryWrapper<User>().eq("user_number", loginLoginPwd.getUserNumber())
                .eq("password", loginLoginPwd.getPassword()));
        if(null == user) {
            throw new DiyException(500,"密码错误");
        }
       return user;
    }
    ...
    public void loginCheck(LoginDto.Login dto) throws Exception {
         userService.existNumber(dto.getUserNum());
